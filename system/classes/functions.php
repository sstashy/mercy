...
class ZFunctions extends System{
    ...
    public static function convertUserRole() {
        ...
        if ($userRoleInt == 1) {
            $roleName = "MERCY ADMİN";
        }
        if ($userRoleInt == 2) {
            $roleName = "Haftalık Freemium";
        } 
        if ($userRoleInt == 3) {
            $roleName = "Aylık VİP";
        } 
    ...
    public static function registeruser(){
        $check = self::table('users')->where('userName', $_POST['username'])->first();
        if($check) {
            return "false";
        }
